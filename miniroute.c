...
int miniroute_send_pkt(network_address_t dest_address, int hdr_len, char* hdr, int data_len, char* data)
{
	...
	/*printf("[DEBUG] DATA packet destined for: ");
	network_printaddr(dest_address);
	printf(", being sent to: ");
	network_printaddr(cached_path->path[1]);
	printf(".\n");*/

	bytes_sent_successfully = network_send_pkt(cached_path->path[1], sizeof(struct routing_header), (char*)header, hdr_len + data_len, user_data);
