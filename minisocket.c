...
int minisocket_send_packet_with_retransmission(minisocket_t socket, minimsg_t msg, int len, char type){
	...
	for(num_retries = 0; num_retries < 7; num_retries++){
		...
		registered_alarm_id = register_alarm(100 * 2^num_retries, force_receive_to_exit,socket);
		...
		//We got something, de-register the alarm
		
		deregister_alarm(registered_alarm_id);
		set_interrupt_level(interrupt_level);
...
int minisocket_receive(minisocket_t socket, minimsg_t msg, int max_len, minisocket_error *error)
{
	...
	while(bytes_received == 0){
		printf("[DEBUG] [In recieve for %s] Waiting for something to arrive. Our Seq is %d, ACk is %d\n",(socket->socket_type == SERVER ? "Server":"Client"),socket->seq_num,socket->ack_num);
		// Wait for packet arrival
		semaphore_P(socket->data_lock);

		printf("[DEBUG] [In recieve for %s] We woke up!\n",(socket->socket_type == SERVER ? "Server":"Client"));
		...
		printf("[DEBUG] [In recieve for %s] Checking the seq/ack numbers\n",(socket->socket_type == SERVER ? "Server":"Client"));
	
		printf("[DEBUG] [In receive for %s] Packet has Seq = %d, ACK = %d\n",(socket->socket_type == SERVER ? "Server":"Client"),unpack_unsigned_int(data_received->buffer + 22),unpack_unsigned_int(data_received->buffer + 26) );
			
		if (received_seq_number != socket->ack_num + 1 || data_received->size - sizeof(struct mini_header_reliable) < 0){
			printf("[DEBUG] [In recieve for %s] Tests failed, re-ending acknoledgment for ACK (%d)\n",(socket->socket_type == SERVER ? "Server":"Client"),socket->ack_num);
			...
			printf("[DEBUG] [In recieve for %s] seq check passed. We received %d bytes \n",(socket->socket_type == SERVER ? "Server":"Client"), bytes_received);
			socket->ack_num++;
			printf("[DEBUG] [In recieve for %s] Sending acknoledgment for ACK (%d)\n",(socket->socket_type == SERVER ? "Server":"Client"),socket->ack_num);
			minisocket_send_packet_without_retransmission(socket, "This is an ACK\n", 15, MSG_ACK);	
